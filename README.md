...
- **Canvases.** A section can be shown as a canvas — pictures and notes on a
  ...
  **A picture on a canvas** can be copied, saved elsewhere or shown in its
  folder from its own menu, the same as one in a note. Ctrl+I turns the
...
the plugin DLLs and `data/` from disk — so this unpacks the folder to
`%LOCALAPPDATA%\ActionNotes\<version>` the first time it runs and starts the
